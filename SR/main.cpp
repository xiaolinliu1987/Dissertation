...
@param lowerlimit A double representing untitledthe lower limit on global distance.
@param upperlimit A double representing the upper limit on global distance.
@param applyLimits A bool representing whether or not to apply limits to our averaging.
@return double The BIC score for the section under the model indicated in the input.
*/
SymInfo calculations_loop(MAP& countList,DMAP& dGlobalList,bool asymm,
    ...
    for(it = qList.begin();it != qList.end();++it){
        ...
        //Winston
        if(dList[it->first]!= 0 && std::isfinite(dList[it->first])){
            localMT += dList[it->first];
            validFamSize++;
        }
    ...
    // TODO : Change global from true to false. Original:  pHatsList = calculate_pHats(avgQ,dList,dGlobalList,true,lowerlimit,upperlimit,applyLimits);
    pHatsList = calculate_pHats(avgQ,dList,dGlobalList, false,lowerlimit,upperlimit,applyLimits);
    numFamiliesUsed = pHatsList.size();
    //Winston
    if(validFamSize != numFamiliesUsed) std::cout<<"Warning : size of families unmatched!"<<std::endl;
    localMT /= validFamSize;
    SymInfo info;
    ...
    if(asymm){
        ...
        info.lambda = avgQ(0,1) + avgQ(0,2) + avgQ(1,0) + avgQ(1,3);  //q1+q5+q4+q6
        if(info.lambda != 0) info.gcEqu =  (avgQ(0,1) + avgQ(0,2)) / info.lambda;
//        std::cout << avgQ(0,1) <<" "<<avgQ(1,0)<<" "<<avgQ(0,2)<<" "<<avgQ(1,3)<<std::endl;
    }
    info.BICVal = BICVal;
    info.mt = localMT;
    return info;
...
std::map<std::string,std::string> findRegionType(std::ifstream& myfile,std::ostream& outfile,DMAP& dGlobalList,
    ...
    std::vector<std::string> symSections;
    std::vector<double> symLamList;
    std::vector<double> symMTList;
    std::vector<double> gcEquList;
    ...
    if(myfile.is_open()){
        ...
        while(!myfile.eof()){
            ...
            if(!myfile.eof()){
                ...
                SymInfo aInfo = calculations_loop(countDictionary,dGlobalList,true,lowerlimit,upperlimit,applyLimits,qbA,numFamUsedA);
                BICValA = aInfo.BICVal;
                //calculate under symmetric model:
                SymInfo sInfo = calculations_loop(countDictionary,dGlobalList,false,lowerlimit,upperlimit,applyLimits,qbS,numFamUsedS);
                BICValS = sInfo.BICVal;
                ...
                if(BICValA==0 && BICValS==0) {   //in case limits cause no families
                ...
                }else if(BICValS > BICValA){//asymmetric case
                    model += "A";
                    ...
                    model += "S";
                    results.emplace(sectionName,model);
                    symSections.push_back(sectionName);
                    symLamList.push_back(sInfo.lambda);
                    symMTList.push_back(sInfo.mt);
                    gcEquList.push_back(sInfo.gcEqu);
                    s_count++;
                }
            }
        }
        ...
        int validMTSize = 0;
        double avgMT = 0.0;
        for (int mtIdx = 0; mtIdx < symMTList.size(); ++mtIdx) {
            if(std::isfinite(symMTList[mtIdx]) && symMTList[mtIdx] != 0) {
                validMTSize++;
                avgMT += symMTList[mtIdx];
            }
        }
        avgMT /= validMTSize;
        ...
        int validEquSize = 0;
        double avgEqu = 0.0;
        for (int equIdx = 0; equIdx < gcEquList.size(); ++equIdx) {
            if(gcEquList[equIdx] != 0){
                validEquSize++;
                avgEqu += gcEquList[equIdx];
            }
        }
        avgEqu /= validEquSize;
        std::cout << "Average GC Equilibrium : "<<avgEqu<<std::endl;

        if(write2File){
            std::ofstream srFile("sr_gc.csv");
            srFile<<"Partition,GC Content,lambda\n";
            Sequence sequence("sequence.fa");
            sequence.read();

            std::unordered_map<char, int> cntMap;
            long sp, ep;
            int globalIdx = 0;
            for (size_t i = 0; i < symSections.size(); ++i) {
                std::string curSection = symSections[i];
                size_t dashPos = curSection.find('-');
                sp = atol(curSection.substr(0, dashPos).c_str());
                ep = atol(curSection.substr(dashPos + 1).c_str());
                ...
                cntMap['G'] = 0;
                cntMap['N'] = 0;
                for (long m = sp; m <= ep; m++) cntMap[toupper(sequence[m])]++;
                double tmpTotalCnt = cntMap['A'] + cntMap['T'] + cntMap['C'] + cntMap['G'];
                double localGCContent = tmpTotalCnt == 0.0 ? 0 : (1.0 * (cntMap['G'] + cntMap['C'])) / tmpTotalCnt;
                double curDist = symLamList[i] * symMTList[i] / avgMT;

                srFile<<curSection<<","<<localGCContent<<","<<curDist<<"\n";
                globalIdx++;
            }
        }
    }else{
    ...
    if(!applyLimits) {
        std::cout << "There are " << as_count << " asymmetric regions using all families." << std::endl;
        std::cout << "There are " << s_count << " symmetric regions using all families." << std::endl;
    }
    else
        std::cout << "There are " << as_count << " asymmetric regions using only families between the distances: "
...
int main(int argc, char *argv[]){
    ...
    std::ofstream outfile(argv[3]); //file for asymmetric findings
    std::ofstream outfile2(argv[4]); //readable global d file
